...
def show_face_guide():
    """얼굴/증명사진을 어디에 맞춰 찍어야 하는지 보여주는 고정 가이드라인(파일 업로드용)."""
    svg = (
        '<svg width="220" height="260" viewBox="0 0 220 260">'
        '<ellipse cx="110" cy="120" rx="80" ry="105" fill="none" stroke="#3fb950" stroke-width="3" stroke-dasharray="8,6" />'
        '<circle cx="80" cy="95" r="5" fill="#3fb950" />'
        '<circle cx="140" cy="95" r="5" fill="#3fb950" />'
        '<circle cx="110" cy="125" r="4" fill="#3fb950" />'
        '<path d="M 85 160 Q 110 175 135 160" fill="none" stroke="#3fb950" stroke-width="3" />'
        '</svg>'
    )
    html = (
        f'<div style="display:flex; justify-content:center; margin-bottom:10px;">{svg}</div>'
        '<p style="text-align:center; color:#8b949e; font-size:0.85rem; margin-top:-8px;">'
        '얼굴(또는 증명사진)을 위 윤곽선 안에 맞춰서 업로드해주세요.</p>'
    )
    st.markdown(html, unsafe_allow_html=True)


def release_camera(key_prefix: str) -> None:
    """해당 key_prefix로 열려 있는 카메라가 있으면 닫습니다 (페이지 이탈 시 호출)."""
...
def _live_preview_fragment(key_prefix: str):
    """0.3초마다 한 프레임씩 읽어 윤곽선을 그려 보여주고, '지금 촬영' 클릭 시 그 프레임을 캡처합니다.
    실시간 미리보기는 가벼운 검출기(Haar Cascade)를 써서 끊김/지연을 줄입니다."""
...
def live_face_capture(key_prefix: str, disabled: bool = False):
    ...
    사용자가 원하는 순간 '지금 촬영' 버튼을 눌러 그 프레임을 캡처합니다."""
    captured_key = f"{key_prefix}_captured_frame"
    previewing_key = f"{key_prefix}_previewing"
    cap_key = f"{key_prefix}_cap"
...
def show_system_settings():
    st.markdown(
        "<div style='font-size: 1.8rem; font-weight: 700; color: #e6edf3; margin-bottom: 20px;'>⚙️ 시스템 설정</div>",
        unsafe_allow_html=True)
    ...
    st.subheader("얼굴 로그인 시도 로그")
    from db import get_login_logs
    logs = get_login_logs(limit=200)
    if not logs:
        st.info("아직 기록된 로그인 시도가 없습니다.")
    else:
        log_df = pd.DataFrame(logs)
        log_df["success"] = log_df["success"].map({1: "성공", 0: "실패"})
        st.dataframe(log_df, use_container_width=True)

    st.markdown("---")
    st.subheader("얼굴 정보 삭제 (개인정보 삭제 요청 대응)")
    from db import get_all_users, delete_user_face

    users = get_all_users()
    face_registered_users = [u for u in users if u["has_face"]]

    if not face_registered_users:
        st.info("얼굴이 등록된 계정이 없습니다.")
    else:
        options = {f"{u['user_id']} ({u['user_name']})": u["user_id"] for u in face_registered_users}
        selected_label = st.selectbox("얼굴 정보를 삭제할 계정", list(options.keys()))
        target_user_id = options[selected_label]

        if st.button(f"'{target_user_id}' 얼굴 정보 삭제", type="primary"):
            delete_user_face(target_user_id)
            st.success(f"'{target_user_id}' 계정의 얼굴 임베딩이 삭제되었습니다. 다음 로그인 시 얼굴을 다시 등록해야 합니다.")
            st.rerun()
