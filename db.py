...
def seed_demo_accounts() -> None:
    """OTT 서비스별 관리자 데모 계정을 DB에 1회 생성합니다(얼굴은 별도 등록 필요).
    role 컬럼은 서비스 구분자(ALL/YouTube/Netflix/Tving)로 사용되어 로그인 후 메뉴/권한을 결정합니다."""
    demo_accounts = {
        "admin": ("admin123", "전체관리자", "ALL"),
        "youtube": ("youtube123", "유튜브관리자", "YouTube"),
        "netflix": ("netflix123", "넷플릭스관리자", "Netflix"),
        "tving": ("tving123", "티빙관리자", "Tving"),
    }
...
def verify_user_password(user_id: str, password: str) -> Tuple[bool, Optional[dict]]:
    init_db()
    conn = get_connection()
    ...
    if not row:
        return False, {"message": "등록되지 않은 아이디입니다."}

    _, input_hash = hash_password(password, salt=row["password_salt"])

    if not secrets.compare_digest(input_hash, row["password_hash"]):
        return False, {"message": "암호가 일치하지 않습니다."}

    # 성공 시 데이터를 딕셔너리로 묶어서 반환
    return True, {
        "name": row["user_name"],
        "role": row["role"],
        "message": "아이디와 암호 확인 완료."
    }


# db.py의 get_user_face_embedding 함수 수정
