...
def get_fast_face_detector() -> cv2.CascadeClassifier:
    ...
    일부 Windows 환경(사용자명에 한글 등 비-ASCII 문자가 포함된 경우)에서는 OpenCV가
    그 경로의 파일을 못 읽는 버그가 있어, 한글이 섞이지 않은 경로로 복사해두고 그걸 사용합니다.
    """
    ...
    if _HAAR_CASCADE is None:
        ...
        safe_dir = Path(os.environ.get("SystemDrive", "C:") + "/") / "_face_login_cache"
        safe_dir.mkdir(parents=True, exist_ok=True)
        safe_path = safe_dir / "haarcascade_frontalface_default.xml"
        if not safe_path.exists():
            shutil.copyfile(original_path, safe_path)
        _HAAR_CASCADE = cv2.CascadeClassifier(str(safe_path))
